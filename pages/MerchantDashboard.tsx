...
const MerchantDashboard: React.FC = () => {
  const { currentMessages, addMessage, addTicket, clearMessages } =
    useTickets();
  ...
  const handleSendMessage = async (text: string) => {
    ...
    // Call Gemini
    const responseText = await merchantFirstAidService.sendMessage(
      text,
      (tool, result) => {
  ...
  const handleCreateTicket = async () => {
    ...
    const summary = await merchantFirstAidService.generateTicketSummary(
      historyForAi
    );
  ...
  return (
    <div className="h-full flex flex-col md:flex-row bg-gray-100">
      ...
      <div className="hidden md:flex flex-col w-64 bg-[#5f259f] text-purple-100">
        <div className="p-4 border-b border-purple-800">
          <h1 className="text-xl font-bold text-white">PhonePe</h1>
          <p className="text-xs text-purple-200">Business Dashboard</p>
        </div>
        <nav className="flex-1 p-4 space-y-2">
          <div className="px-4 py-2 bg-purple-800 text-white rounded cursor-pointer font-medium">
            Home
          </div>
          <div className="px-4 py-2 hover:bg-purple-800 rounded cursor-pointer transition-colors">
            Transactions
          </div>
          <div className="px-4 py-2 hover:bg-purple-800 rounded cursor-pointer transition-colors">
            Settlements
        ...
        <div className="p-4 border-t border-purple-800">
          <button
            ...
            className="w-full text-xs text-center text-purple-300 hover:text-white transition-colors"
          >
